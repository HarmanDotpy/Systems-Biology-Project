...
%% collecting timeperiod data as function of beta by solving the dde multiple times using the dde3 function
num_iter = 101;
avgperiod_laci = zeros(1,num_iter);
avgperiod_arac = zeros(1,num_iter);
range_beta = 0: 0.005 : 0.5;
for j = 1:num_iter
    Parameter_Vec = [alpha_r; alpha_a; gamma_r; gamma_a; f; Ca; Cr_T; N; range_beta(j); Ro; A_T];
...
plot(range_beta,avgperiod_laci,'LineWidth', 2,'DisplayName', 'TimePeriod lacI');
hold on
plot(range_beta,avgperiod_arac,'LineWidth', 2,'DisplayName', 'TimePeriod AraC');
legend;
xlabel('Different values of beta');
