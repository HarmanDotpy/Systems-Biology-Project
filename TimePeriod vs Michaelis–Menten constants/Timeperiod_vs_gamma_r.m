...
%% collecting timeperiod data as function of gamma_r by solving the dde multiple times using the dde3 function
%please keep num iterations equal to the sze of the array range_gamma_r
num_iter = 131;
avgperiod_laci = zeros(1,num_iter);
avgperiod_arac = zeros(1,num_iter);
range_gamma_r= 20: 1: 150;
for j = 1:num_iter
    Parameter_Vec = [alpha_r; alpha_a; range_gamma_r(j); gamma_a; f; Ca; Cr_T; N; beta; Ro; A_T];
...
plot(range_gamma_r,avgperiod_laci,'LineWidth', 2,'DisplayName', 'TimePeriod lacI');
hold on
plot(range_gamma_r,avgperiod_arac,'LineWidth', 2,'DisplayName', 'TimePeriod AraC');
legend;
xlabel('Different values of gamma_r');
